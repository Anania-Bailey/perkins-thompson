...
jQuery(function($) {
  ...
  $(document).ready(function() {
    ...
    resizeBox();
    disclaimerPopup();
    $('.pt-accordion input[type="radio"]').on('change', function() {
        var $accordion = $(this).closest('.pt-accordion');
        var $accordionContent = $accordion.find('.accordion-content');
        var elementTop = $accordion.offset().top;
        var scrollDuration = 100; // Adjust to match your CSS transition duration
    
        if ($(this).is(':checked')) {
            // Start expanding
            $accordionContent.addClass('expanding');
    
            // Update scroll position during expansion
            var interval = setInterval(function() {
                var currentTop = $accordion.offset().top;
                if (($(window).scrollTop() + 150) > currentTop) {
                  $(window).scrollTop((currentTop - 150));
                }
                elementTop = currentTop; // Update elementTop for next scroll adjustment
            }, 20); // Adjust the interval timing as needed
    
            // Clear interval after expansion is complete
            setTimeout(function() {
                $accordionContent.removeClass('expanding');
                clearInterval(interval);
            }, scrollDuration); // Ensure this matches the duration of the CSS transition
        } else {
            $accordionContent.removeClass('expanding');
        }
    });
